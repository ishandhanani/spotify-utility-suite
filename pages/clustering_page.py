...
st.write("This program allows you to cluster your favorite artists and compare them to the 'This is: <Artist>' playlists created by Spotify. This page requires 1 csv files formatted in the following: ")
st.write("artist_name | personal_artist_playlist_link |spotify_this_is_artist_playlist_link", )
...
def read_file():
    if file is not None:
        ...
        with st.spinner("Creating personal artist matrix. May take a while depending on playlist size"):
            pmatrix = su.create_feature_matrix(personaldf)
        with st.spinner("Creating spotify artist matrix. May take a while depending on playlist size"):
...
st.write("For visualization purposes, we use two principal components which explain about 64% of the variance. Note that to visualize the components,")
